...
import { wantToLiveCountries, wishToGoCountries } from "../recoil/atoms";
...
interface HaveBeenToCountryProps {
  country: string;
}
function HaveBeenToCountry({ country }: HaveBeenToCountryProps) {
  const isSameCountry: CompareProc<string> = makeSame<string>(
    (arg1, arg2) => arg1 === arg2
  );
  const [wishCountries, haveBeenToCountries, wantCountries] = [
    useRecoilState(wishToGoCountries),
    useRecoilState(alreadyGoneCountries),
    useRecoilState(wantToLiveCountries)
  ];
  const switchWishHaveCountries = switchState<string>(
    isSameCountry,
    wishCountries,
    haveBeenToCountries
  );
  ...
  const onSwitchWishHaveClick = () => {
    switchWishHaveCountries(country);
  };
  ...
  return (
    <List>
      ...
      <Button onClick={onSwitchWishHaveClick}>
        <span role="img">❌</span>
      </Button>
    </List>
  );
...
export default HaveBeenToCountry;
